...
TEST_CASE("Cooler: dataset random iteration", "[dataset][medium]") {
  ...
  Dataset dset(grp, "int", std::uint16_t{}, HighFive::DataSpace::UNLIMITED,
  ...
  std::vector<std::uint16_t> buff(N);
  std::generate(buff.begin(), buff.end(),
                [&]() { return std::uniform_int_distribution<std::uint16_t>{0, 65535}(rand_eng); });
  dset.append(buff);
  REQUIRE(dset.size() == N);

  SECTION("operator -/+") {
    const auto first = dset.begin<std::uint16_t>(32'000);
    const auto mid = first + M;
    const auto last = dset.end<std::uint16_t>(32'000);
  ...
  SECTION("subsequent calls to operator+=") {
    for (std::size_t i = 0; i < 50; ++i) {
      auto first = dset.begin<std::uint16_t>(32'000);
      auto last = dset.end<std::uint16_t>(32'000);
      std::ptrdiff_t j = 0;
  ...
  SECTION("subsequent calls to operator-=") {
    for (std::size_t i = 0; i < 50; ++i) {
      auto first = dset.end<std::uint16_t>(32'000) - 1;
      auto last = dset.begin<std::uint16_t>(32'000);
