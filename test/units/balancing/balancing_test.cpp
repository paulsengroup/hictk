...
TEST_CASE("Balancing: VC", "[balancing][short]") {
  ...
  auto hf = hictk::hic::HiCFile(path.string(), 2500000);
  ...
  SECTION("INTRA") {
    ...
    for (const auto& chrom : hf.chromosomes()) {
      ...
      auto sel = hf.fetch(chrom.name());
      const auto num_bins = hf.bins().subset(chrom).size();
      const auto bin_id_offset = hf.bins().at(chrom.name(), 0).id();
      const auto w =
          hictk::balancing::VC<std::int32_t>(sel.begin<std::int32_t>(), sel.end<std::int32_t>(),
                                             num_bins, bin_id_offset)
              .get_weights();
      weights.insert(weights.end(), w.begin(), w.end());
    }

    const auto expected = hf.fetch(hic::NormalizationMethod::VC).weights();
  ...
  SECTION("GW") {
    ...
    const auto expected = hf.fetch(hic::NormalizationMethod::GW_VC).weights();
    compare_weights(weights, expected);
  }
}
