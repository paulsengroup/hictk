...
constexpr auto* path_plaintext = "test/data/data.txt";
constexpr auto* path_binary = "test/data/data.zip";
constexpr auto* path = path_plaintext;
...
TEST_CASE("Local - ctor") {
    ...
    SECTION("valid path") {
        const auto s = open_local_file(path_plaintext);
...
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
TEST_CASE("Local - seek") {
    auto s = open_local_file(path_plaintext);
    {
        std::string buff;
        s.read(buff, 1);
        s.seekg(0);
...
TEST_CASE("Local - read binary") {
    auto s = open_local_file(path_binary);
    s.seekg(10);
