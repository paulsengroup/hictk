...
static const TestCaseGenerator test_generator{
    "cooler::File::fetch (trans)",
    std::array<std::string_view, 1>{"test/data/integration_tests/4DNFIZ1ZVXC8.mcool"},
    ...
    std::array<std::string_view, 3>{"chrX:10,200,000-10,300,000", "chrX:5,000,000-6,000,000",
                                    "chrX:15,000,000-20,000,000"},
...
static void run_benchmark() {
  ...
  (Catch::Benchmark::Chronometer meter) {
    ...
    const cooler::File clr(
        fmt::format(FMT_STRING("{}::/resolutions/{}"), params.path.string(), params.resolution));
    if (params.normalization == balancing::Method::NONE()) {
      meter.measure([&]() {
        return count_nnz<std::uint32_t>(clr, params.range1, params.range2, params.normalization);
      });
    } else {
      meter.measure([&]() {
        return count_nnz<double>(clr, params.range1, params.range2, params.normalization);
      });
    }
  };
}
...
void register_cooler_trans_queries_benchmarks() { register_benchmarks(); }
