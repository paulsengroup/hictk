...
GenomicInterval::GenomicInterval(const Chromosome &chrom_) noexcept
    : GenomicInterval(chrom_, 0, chrom_.size()) {}

GenomicInterval::GenomicInterval(Chromosome chrom_, std::uint32_t start_,
...
GenomicInterval::operator bool() const noexcept { return !!chrom(); }
...
const Chromosome &GenomicInterval::chrom() const noexcept { return _chrom; }

std::tuple<std::string, std::uint32_t, std::uint32_t> GenomicInterval::parse(
...
std::tuple<std::string, std::uint32_t, std::uint32_t> GenomicInterval::parse_ucsc(
    std::string buffer) {
  if (buffer.empty()) {
  ...
  if (buffer.find(',', p1) != std::string::npos) {
    buffer.erase(std::remove(buffer.begin() + static_cast<std::ptrdiff_t>(p1), buffer.end(), ','),
                 buffer.end());
...
std::tuple<std::string, std::uint32_t, std::uint32_t> GenomicInterval::parse_bed(
...
GenomicInterval GenomicInterval::parse(const Reference &chroms, const std::string &query,
                                       Type type) {
  if (type == Type::UCSC) {
...
GenomicInterval GenomicInterval::parse_ucsc(const Reference &chroms, std::string buffer) {
  if (buffer.empty()) {
    throw std::runtime_error("query is empty");
  }
...
GenomicInterval GenomicInterval::parse_bed(const Reference &chroms, std::string_view buffer,
                                           char sep) {
  if (buffer.empty()) {
    throw std::runtime_error("interval is empty");
  }
...
std::uint64_t area(const GenomicInterval &gi, std::uint32_t resolution,
                   bool upper_triangular) noexcept {
  return area(gi, gi, resolution, upper_triangular);
}

std::uint64_t area(const GenomicInterval &gi1, const GenomicInterval &gi2, std::uint32_t resolution,
                   bool upper_triangular) noexcept {
...
std::uint64_t area(std::uint32_t start_pos, std::uint32_t end_pos, std::uint32_t resolution,
                   bool upper_triangular) noexcept {
  return area(start_pos, end_pos, start_pos, end_pos, resolution, upper_triangular);
}

std::uint64_t area(std::uint32_t start1_pos, std::uint32_t end1_pos, std::uint32_t start2_pos,
                   std::uint32_t end2_pos, std::uint32_t resolution,
                   bool upper_triangular) noexcept {
...
std::size_t std::hash<hictk::GenomicInterval>::operator()(
