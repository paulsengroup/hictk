...
AtomicBitSet::AtomicBitSet(std::size_t size_, bool value) : _buff(size_ / sizeof(I)), _size(size_) {
  fill(value);
}

AtomicBitSet::AtomicBitSet(const AtomicBitSet& other)
...
AtomicBitSet& AtomicBitSet::operator=(const AtomicBitSet& other) {
  if (this == &other) {
    return *this;
  }
...
void AtomicBitSet::atomic_set(std::size_t i, bool value) noexcept {
  assert(i < _size);
...
bool AtomicBitSet::atomic_test(std::size_t i) const noexcept {
  assert(i < _size);

  return atomic_test(_buff, i);
}

std::size_t AtomicBitSet::size() const noexcept { return _size; }

bool AtomicBitSet::empty() const noexcept { return size() == 0; }

void AtomicBitSet::fill(bool value) noexcept {
  if (value) {
...
void AtomicBitSet::resize(std::size_t size_, bool value) {
  if (size_ != size()) {
...
std::size_t AtomicBitSet::compute_offset(std::size_t i) noexcept { return i / sizeof(I); }

bool AtomicBitSet::atomic_test(const std::vector<std::atomic<I>>& buff, std::size_t i) noexcept {
...
VectorOfAtomicDecimals::VectorOfAtomicDecimals(std::size_t size_, std::uint64_t decimal_bits)
    : _margsi(size_),
      _margsd(size_),
      _nanmask(size_),
      _infmask(size_),
...
VectorOfAtomicDecimals::VectorOfAtomicDecimals(const VectorOfAtomicDecimals& other)
    : _margsi(other.size()),
      _margsd(other.size()),
...
VectorOfAtomicDecimals& VectorOfAtomicDecimals::operator=(const VectorOfAtomicDecimals& other) {
  if (this == &other) {
    return *this;
  }
...
double VectorOfAtomicDecimals::operator[](std::size_t i) const noexcept {
  assert(i < size());
...
void VectorOfAtomicDecimals::atomic_add(std::size_t i, double n) noexcept {
  assert(i < size());
...
void VectorOfAtomicDecimals::set(std::size_t i, double n) noexcept {
  assert(i < size());
...
void VectorOfAtomicDecimals::multiply(const std::vector<double>& v) noexcept {
  assert(size() == v.size());
...
const std::vector<double>& VectorOfAtomicDecimals::operator()() const noexcept {
...
std::vector<double>& VectorOfAtomicDecimals::operator()() noexcept {
  assert(_margsi.size() == _margsd.size());
  for (std::size_t i = 0; i < size(); ++i) {
    _margsd[i] = (*this)[i];
  }
...
void VectorOfAtomicDecimals::fill(double value) noexcept {
  for (auto& n : _margsi) {
    n = encode(value);
  }
...
void VectorOfAtomicDecimals::resize(std::size_t size_, double value) {
  if (size_ != size()) {
...
std::uint8_t VectorOfAtomicDecimals::decimal_bits() const noexcept {
#ifdef _MSC_VER
...
std::size_t VectorOfAtomicDecimals::size() const noexcept { return _margsi.size(); }

bool VectorOfAtomicDecimals::empty() const noexcept { return size() == 0; }

std::pair<double, double> VectorOfAtomicDecimals::domain(bool include_inf) const noexcept {
  if (include_inf) {
...
auto VectorOfAtomicDecimals::encode(double n) const noexcept -> I {
...
double VectorOfAtomicDecimals::decode(I n) const noexcept { return static_cast<double>(n) / _cfxd; }

double VectorOfAtomicDecimals::compute_max_value(std::uint8_t decimal_bits) noexcept {
  assert(decimal_bits < 64);
  return std::nextafter(static_cast<double>(std::numeric_limits<I>::max() >> decimal_bits), 0.0);
}

bool SparseMatrix::empty() const noexcept { return size() == 0; }
std::size_t SparseMatrix::size() const noexcept { return _counts.size(); }

void SparseMatrix::clear(bool shrink_to_fit_) noexcept {
  _bin1_ids.clear();
  _bin2_ids.clear();
  _counts.clear();
  if (shrink_to_fit_) {
...
void SparseMatrix::reserve(std::size_t capacity) {
  _bin1_ids.reserve(capacity);
  _bin2_ids.reserve(capacity);
  _counts.reserve(capacity);
}

void SparseMatrix::shrink_to_fit() noexcept {
  _bin1_ids.shrink_to_fit();
  _bin2_ids.shrink_to_fit();
  _counts.shrink_to_fit();
}

void SparseMatrix::finalize() { shrink_to_fit(); }

const std::vector<std::uint64_t>& SparseMatrix::bin1_ids() const noexcept { return _bin1_ids; }
const std::vector<std::uint64_t>& SparseMatrix::bin2_ids() const noexcept { return _bin2_ids; }
const std::vector<double>& SparseMatrix::counts() const noexcept { return _counts; }

void SparseMatrix::push_back(std::uint64_t bin1_id, std::uint64_t bin2_id, double count,
...
void SparseMatrix::serialize(filestream::FileStream& fs, std::string& tmpbuff, ZSTD_CCtx& ctx,
                             int compression_lvl) const {
...
void SparseMatrix::deserialize(filestream::FileStream& fs, std::string& tmpbuff, ZSTD_DCtx& ctx) {
  ...
  auto compressed_size = fs.unsafe_read<std::size_t>();
...
void SparseMatrix::marginalize(VectorOfAtomicDecimals& marg, bool init_buffer) const {
  assert(!marg.empty());
  if (init_buffer) {
    marg.fill(0);
  }
...
void SparseMatrix::marginalize_nnz(VectorOfAtomicDecimals& marg, bool init_buffer) const {
  if (init_buffer) {
    marg.fill(0);
  }
...
void SparseMatrix::times_outer_product_marg(VectorOfAtomicDecimals& marg,
...
void SparseMatrix::multiply(VectorOfAtomicDecimals& buffer, nonstd::span<const double> cfx,
                            bool init_buffer) const {
...
double SparseMatrix::compute_scaling_factor_for_scale(const std::vector<double>& weights) const {
  if (empty()) {
    return std::numeric_limits<double>::quiet_NaN();
  }
...
SparseMatrixChunked::SparseMatrixChunked(std::size_t chunk_size)
    : _chunks(1), _chunk_size(chunk_size) {
  assert(chunk_size != 0);
  _chunks.back().reserve(_chunk_size);
}

bool SparseMatrixChunked::empty() const noexcept { return size() == 0; }

std::size_t SparseMatrixChunked::size() const noexcept { return _size; }

std::size_t SparseMatrixChunked::num_chunks() const noexcept {
  return empty() ? std::size_t{} : _chunks.size();
}

std::size_t SparseMatrixChunked::chunk_size() const noexcept { return _chunk_size; }

void SparseMatrixChunked::shrink_to_fit() noexcept {
  assert(!_chunks.empty());
  _chunks.back().shrink_to_fit();
}

void SparseMatrixChunked::clear(bool shrink_to_fit_) {
  _chunks.resize(1);
  _chunks.back().clear();
  _size = 0;
...
void SparseMatrixChunked::push_back(std::uint64_t bin1_id, std::uint64_t bin2_id, double count,
                                    std::size_t bin_offset) {
  assert(!_chunks.empty());
...
void SparseMatrixChunked::finalize() { shrink_to_fit(); }

void SparseMatrixChunked::marginalize(VectorOfAtomicDecimals& marg, BS::light_thread_pool* tpool,
                                      bool init_buffer) const {
...
void SparseMatrixChunked::marginalize_nnz(VectorOfAtomicDecimals& marg,
...
void SparseMatrixChunked::times_outer_product_marg(VectorOfAtomicDecimals& marg,
                                                   nonstd::span<const double> biases,
                                                   nonstd::span<const double> weights,
...
void SparseMatrixChunked::multiply(VectorOfAtomicDecimals& buffer, nonstd::span<const double> cfx,
                                   BS::light_thread_pool* tpool, bool init_buffer) const {
...
double SparseMatrixChunked::compute_scaling_factor_for_scale(
...
FileBackedSparseMatrix::FileBackedSparseMatrix(std::filesystem::path tmp_file,
...
FileBackedSparseMatrix::~FileBackedSparseMatrix() noexcept {
  try {
...
bool FileBackedSparseMatrix::empty() const noexcept { return size() == 0; }
std::size_t FileBackedSparseMatrix::size() const noexcept { return _size; }
void FileBackedSparseMatrix::clear(bool shrink_to_fit_) {
  _index.clear();
...
void FileBackedSparseMatrix::push_back(std::uint64_t bin1_id, std::uint64_t bin2_id, double count,
                                       std::size_t bin_offset) {
...
void FileBackedSparseMatrix::finalize() {
  if (!_matrix.empty()) {
    write_chunk();
  }
...
void FileBackedSparseMatrix::marginalize(VectorOfAtomicDecimals& marg, BS::light_thread_pool* tpool,
                                         bool init_buffer) const {
  auto marginalize_impl = [&](std::size_t istart, std::size_t iend) {
...
void FileBackedSparseMatrix::marginalize_nnz(VectorOfAtomicDecimals& marg,
                                             BS::light_thread_pool* tpool, bool init_buffer) const {
  auto marginalize_nnz_impl = [&](std::size_t istart, std::size_t iend) {
    std::unique_ptr<ZSTD_DCtx_s> zstd_dctx(ZSTD_createDCtx());
    filestream::FileStream fs(_path.string(), nullptr);  // opening wo/ locking is ok
...
void FileBackedSparseMatrix::times_outer_product_marg(VectorOfAtomicDecimals& marg,
                                                      nonstd::span<const double> biases,
                                                      nonstd::span<const double> weights,
                                                      BS::light_thread_pool* tpool,
                                                      bool init_buffer) const {
...
void FileBackedSparseMatrix::multiply(VectorOfAtomicDecimals& buffer,
                                      nonstd::span<const double> cfx, BS::light_thread_pool* tpool,
                                      bool init_buffer) const {
  auto multiply_impl = [&](std::size_t istart, std::size_t iend) {
    std::unique_ptr<ZSTD_DCtx_s> zstd_dctx(ZSTD_createDCtx());
    filestream::FileStream fs(_path.string(), nullptr);  // opening wo/ locking is ok
...
double FileBackedSparseMatrix::compute_scaling_factor_for_scale(
    const std::vector<double>& weights) const {
  if (empty()) {
    return std::numeric_limits<double>::quiet_NaN();
  }
...
void FileBackedSparseMatrix::write_chunk() {
  assert(!_matrix.empty());
  _index.push_back(_fs.unsafe_tellp());
  _matrix.finalize();
  _matrix.serialize(_fs, _buff, *_zstd_cctx, _compression_lvl);
  _matrix.clear();
}

std::vector<std::size_t> FileBackedSparseMatrix::compute_chunk_offsets(std::size_t size,
