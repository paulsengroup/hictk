...
std::streampos HiCSectionOffsets::start() const noexcept { return _position; }

std::streampos HiCSectionOffsets::end() const noexcept {
  return _position + static_cast<std::streamoff>(size());
}

std::size_t HiCSectionOffsets::size() const noexcept { return _size; }

void HiCSectionOffsets::extend(std::size_t s) noexcept { _size += s; }
void HiCSectionOffsets::extend(std::streamoff s) noexcept { extend(static_cast<std::size_t>(s)); }

void HiCSectionOffsets::set_size(std::size_t new_size) noexcept { _size = new_size; }

bool BlockIndexKey::operator<(const BlockIndexKey &other) const noexcept {
...
bool MatrixBodyMetadataTank::Key::operator==(const Key &other) const noexcept {
  return chrom1 == other.chrom1 && chrom2 == other.chrom2;
}

bool MatrixBodyMetadataTank::contains(const Chromosome &chrom1,
                                      const Chromosome &chrom2) const noexcept {
  return _tank.contains(Key{chrom1, chrom2});
}

auto MatrixBodyMetadataTank::at(const Chromosome &chrom1, const Chromosome &chrom2) const
...
HiCSectionOffsets MatrixBodyMetadataTank::offset(const Chromosome &chrom1,
...
void MatrixBodyMetadataTank::insert(const Chromosome &chrom1, const Chromosome &chrom2,
...
void MatrixBodyMetadataTank::update_offsets(const Chromosome &chrom1, const Chromosome &chrom2,
...
void MatrixBodyMetadataTank::remove(const Chromosome &chrom1, const Chromosome &chrom2) {
  try {
...
auto MatrixBodyMetadataTank::operator()() const noexcept
    -> const phmap::flat_hash_map<Key, MatrixBodyMetadata> & {
  return _tank;
}

HiCFileWriter::HiCFileWriter(std::string_view path_, std::size_t n_threads)
    : _fs(std::string{path_}, std::make_shared<std::mutex>(), std::ios::in | std::ios::out),
      ...
      _block_index_mtx(std::make_unique<std::mutex>()),
...
HiCFileWriter::HiCFileWriter(std::string_view path_, Reference chromosomes_,
                             std::vector<std::uint32_t> resolutions_, std::string_view assembly_,
                             std::size_t n_threads, std::size_t chunk_size,
                             std::filesystem::path tmpdir, std::uint32_t compression_lvl,
...
std::string_view HiCFileWriter::path() const noexcept { return _header.url; }

const Reference &HiCFileWriter::chromosomes() const noexcept { return _header.chromosomes; }

const BinTable &HiCFileWriter::bins(std::uint32_t resolution) const {
...
auto HiCFileWriter::stats(std::uint32_t resolution) const noexcept -> Stats {
...
void HiCFileWriter::write_header() {
...
void HiCFileWriter::write_footer_size() {
...
void HiCFileWriter::write_footer_offset() {
...
void HiCFileWriter::write_norm_vector_index() {
  const auto offset =
...
void HiCFileWriter::write_pixels(bool skip_all_vs_all_matrix) {
...
void HiCFileWriter::write_all_matrix(std::uint32_t target_num_bins) {
  try {
...
auto HiCFileWriter::write_pixels(const Chromosome &chrom1, const Chromosome &chrom2)
    -> HiCSectionOffsets {
  try {
...
void HiCFileWriter::write_body_metadata() {
  const auto section_start = _data_block_section.end();
...
void HiCFileWriter::add_body_metadata(std::uint32_t resolution, const Chromosome &chrom1,
...
void HiCFileWriter::write_footers() {
...
void HiCFileWriter::add_footer(const Chromosome &chrom1, const Chromosome &chrom2) {
...
void HiCFileWriter::write_norm_vectors_and_norm_expected_values() {
...
HiCSectionOffsets HiCFileWriter::write_empty_expected_values() {
...
HiCSectionOffsets HiCFileWriter::write_empty_normalized_expected_values() {
...
ExpectedValuesBlock HiCFileWriter::compute_expected_values(std::uint32_t resolution) {
...
NormalizedExpectedValuesBlock HiCFileWriter::compute_normalized_expected_values(
...
HiCSectionOffsets HiCFileWriter::compute_and_write_expected_values() {
  assert(_tpool);
...
HiCSectionOffsets HiCFileWriter::compute_and_write_normalized_expected_values() {
  assert(_tpool);
  assert(_tpool->get_thread_count() != 0);
...
void HiCFileWriter::add_norm_vector(const NormalizationVectorIndexBlock &blk,
...
void HiCFileWriter::add_norm_vector(std::string_view type, const Chromosome &chrom,
                                    std::string_view unit, std::uint32_t bin_size,
                                    const Weights &weights, bool force_overwrite,
...
void HiCFileWriter::add_norm_vector(const NormalizationVectorIndexBlock &blk,
  ...
  std::transform(weights.begin(Weights::Type::DIVISIVE), weights.end(Weights::Type::DIVISIVE),
                 weights_f.begin(), [](const auto n) { return static_cast<float>(n); });
  add_norm_vector(blk, weights_f, force_overwrite);
}

void HiCFileWriter::add_norm_vector(std::string_view type, std::string_view unit,
                                    std::uint32_t bin_size, const Weights &weights,
  ...
  try {
    ...
    std::ptrdiff_t i0 = 0;
    std::ptrdiff_t i1 = 0;
    const auto weights_ = weights(Weights::Type::DIVISIVE);
    for (const auto &chrom : chromosomes()) {
      if (chrom.is_all()) {
        continue;
      }
      ...
      add_norm_vector(type, chrom, unit, bin_size,
                      {std::move(chrom_weights), Weights::Type::DIVISIVE}, force_overwrite);
      i0 = i1;
    }
  } catch (const std::exception &e) {
...
void HiCFileWriter::finalize(bool compute_expected_values) {
  try {
    if (_block_index.empty()) {
...
HiCSectionOffsets HiCFileWriter::write_norm_vectors() {
  try {
    const auto offset1 =
...
HiCHeader HiCFileWriter::read_header(filestream::FileStream &fs) {
  return HiCHeader::deserialize(0, fs);
}

HiCHeader HiCFileWriter::init_header(std::string_view path, Reference chromosomes,
                                     std::vector<std::uint32_t> resolutions,
                                     std::string_view assembly, bool skip_all_vs_all_matrix) {
...
auto HiCFileWriter::init_bin_tables(const Reference &chromosomes,
                                    const std::vector<std::uint32_t> &resolutions) -> BinTables {
...
auto HiCFileWriter::init_interaction_block_mappers(const std::filesystem::path &root_folder,
...
std::unique_ptr<BS::light_thread_pool> HiCFileWriter::init_tpool(std::size_t n_threads) {
  return std::make_unique<BS::light_thread_pool>(n_threads < 2 ? std::size_t{1} : n_threads);
}

HiCSectionOffsets HiCFileWriter::write_pixels(const Chromosome &chrom1, const Chromosome &chrom2,
...
auto HiCFileWriter::write_interaction_blocks(std::streampos offset, const Chromosome &chrom1,
  ...
  try {
    ...
    throw std::runtime_error(
        fmt::format(FMT_STRING("an error occurred while interaction blocks using {} threads: {}"),
                    !_tpool ? 0 : _tpool->get_thread_count(), e.what()));
  }
}

auto HiCFileWriter::write_interaction_block(std::streampos offset, std::uint64_t block_id,
...
std::size_t HiCFileWriter::compute_num_bins(const Chromosome &chrom1, const Chromosome &chrom2,
                                            std::uint32_t resolution) {
  return HiCInteractionToBlockMapper::compute_num_bins(chrom1, chrom2, resolution);
}

void HiCFileWriter::add_norm_expected_values(const NormalizedExpectedValuesBlock &blk,
                                             bool force_overwrite) {
  if (blk.type == "NONE") {
    throw std::runtime_error("caught attempt to write NONE weights");
  }
...
void HiCFileWriter::read_norm_expected_values() {
...
void HiCFileWriter::read_norm_vectors() {
...
std::vector<float> HiCFileWriter::read_norm_vector(const NormalizationVectorIndexBlock &blk) {
  try {
...
void HiCFileWriter::read_offsets() {
  try {
    [[maybe_unused]] const auto lck = _fs.lock();
...
std::size_t HiCFileWriter::compute_block_column_count(const Chromosome &chrom1,
...
auto HiCFileWriter::merge_and_compress_blocks_thr(
...
void HiCFileWriter::write_compressed_blocks(
...
std::size_t std::hash<hictk::hic::internal::MatrixBodyMetadataTank::Key>::operator()(
