...
#include "hictk/hic/hic_file.hpp"
...
File::File(std::string url_, std::optional<std::uint32_t> resolution_, MatrixType type_,
...
File& File::open(std::string url_, std::optional<std::uint32_t> resolution_, MatrixType type_,
                 MatrixUnit unit_, std::uint64_t block_cache_capacity) {
...
File& File::open(std::uint32_t resolution_, MatrixType type_, MatrixUnit unit_,
                 std::uint64_t block_cache_capacity) {
  return open(path(), resolution_, type_, unit_, block_cache_capacity);
}

bool File::has_resolution(std::uint32_t resolution) const {
  const auto match = std::find(avail_resolutions().begin(), avail_resolutions().end(), resolution);
  return match != avail_resolutions().end();
}

const std::string& File::path() const noexcept { return _fs->path(); }

const std::string& File::name() const noexcept { return path(); }

std::int32_t File::version() const noexcept { return _fs->version(); }

const BinTable& File::bins() const noexcept {
  assert(_bins);
  return *_bins;
}

std::shared_ptr<const BinTable> File::bins_ptr() const noexcept { return _bins; }

std::uint32_t File::resolution() const noexcept { return bins().resolution(); }
...
const Reference& File::chromosomes() const noexcept { return bins().chromosomes(); }

const std::string& File::assembly() const noexcept { return _fs->header().genomeID; }

const phmap::flat_hash_map<std::string, std::string>& File::attributes() const noexcept {
  return _fs->header().attributes;
}

const std::vector<std::uint32_t>& File::avail_resolutions() const noexcept {
...
std::shared_ptr<const internal::HiCFooter> File::get_footer(const Chromosome& chrom1,
...
PixelSelectorAll File::fetch(const balancing::Method& norm,
...
PixelSelector File::fetch(std::string_view range, const balancing::Method& norm,
...
PixelSelector File::fetch(const Chromosome& chrom1, std::uint32_t start1, std::uint32_t end1,
...
std::vector<double> File::expected_values(const Chromosome& chrom,
...
std::size_t File::num_cached_footers() const noexcept { return _footers.size(); }

void File::purge_footer_cache() { _footers.clear(); }

double File::block_cache_hit_rate() const noexcept { return _block_cache->hit_rate(); }
void File::reset_cache_stats() const noexcept { _block_cache->reset_stats(); }
void File::clear_cache() noexcept { _block_cache->clear(); }
void File::optimize_cache_size(std::size_t upper_bound) {
  optimize_cache_size_for_random_access(upper_bound);
}

void File::optimize_cache_size_for_iteration(std::size_t upper_bound) {
...
void File::optimize_cache_size_for_random_access(std::size_t upper_bound) {
  // NOLINTBEGIN(*-avoid-magic-numbers)
  if (version() < 9) {
    _block_cache->set_capacity(std::min(upper_bound, std::size_t{10'000'000}));
    return;
...
std::size_t File::cache_capacity() const noexcept { return _block_cache->capacity_bytes(); }

std::size_t File::estimate_cache_size_cis() const {
...
std::size_t File::estimate_cache_size_trans() const {
...
std::uint32_t File::infer_or_validate_resolution(const internal::HiCFileReader& fs,
                                                 std::optional<std::uint32_t> wanted_resolution) {
