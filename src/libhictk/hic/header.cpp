...
bool HiCHeader::operator==(const HiCHeader &other) const noexcept {
  return url == other.url && footerPosition == other.footerPosition;
}

bool HiCHeader::operator!=(const HiCHeader &other) const noexcept { return !(*this == other); }

std::string HiCHeader::serialize(BinaryBuffer &buffer, bool clear) const {
...
HiCHeader HiCHeader::deserialize(std::streampos offset, filestream::FileStream &fs) {
  [[maybe_unused]] const auto lck = fs.lock();
  return unsafe_deserialize(offset, fs);
}

HiCHeader HiCHeader::unsafe_deserialize(std::streampos offset, filestream::FileStream &fs) {
...
std::size_t std::hash<hictk::hic::internal::HiCHeader>::operator()(
    hictk::hic::internal::HiCHeader const &h) const noexcept {
