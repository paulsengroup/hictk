...
inline std::uint64_t PixelSelector::size(bool upper_triangle) const {
  ...
  // the + 1 is required to properly deal with queries that align with the end of a chromosome
  const auto end1 = _coord1.bin2.end() + 1;
  const auto end2 = _coord2.bin2.end() + 1;
