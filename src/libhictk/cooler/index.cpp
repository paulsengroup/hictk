...
Index::Index(std::shared_ptr<const BinTable> bins, const std::vector<std::uint64_t> &chrom_offsets,
             std::uint64_t nnz, bool allocate)
    : _bins(std::move(bins)),
      _idx(init(_bins->chromosomes(), *_bins, chrom_offsets, allocate)),
      _nnz(nnz) {}

const Reference &Index::chromosomes() const noexcept { return bins().chromosomes(); }

const BinTable &Index::bins() const noexcept {
...
std::shared_ptr<const BinTable> Index::bins_ptr() const noexcept { return _bins; }

std::size_t Index::size() const noexcept { return bins().size(); }
...
std::size_t Index::size(std::uint32_t chrom_id) const { return at(chrom_id).size(); }

bool Index::empty() const noexcept { return size() == 0; }

bool Index::empty(std::uint32_t chrom_id) const {
  const auto &idx = at(chrom_id);
  return idx.front() == idx.back();
}

bool Index::empty(std::string_view chrom_name) const {
  return empty(chromosomes().at(chrom_name).id());
}

std::uint32_t Index::resolution() const noexcept {
  if (HICTK_UNLIKELY(!_bins)) {
    return 0;
  }
...
auto Index::cbegin() const noexcept -> const_iterator { return begin(); }
auto Index::cend() const noexcept -> const_iterator { return end(); }
...
auto Index::at(std::uint32_t chrom_id) -> mapped_type & { return _idx.at(chrom_id); }
...
auto Index::at(std::uint32_t chrom_id) const -> const mapped_type & { return _idx.at(chrom_id); }

bool Index::contains(std::uint32_t chrom_id) const noexcept { return _idx.contains(chrom_id); }

bool Index::contains(std::string_view chrom_name) const noexcept {
  try {
    const auto chrom_id = chromosomes().get_id(chrom_name);
...
void Index::set(const Chromosome &chrom, OffsetVect offsets) {
...
void Index::set_offset_by_bin(const Bin &bin, std::uint64_t offset) {
...
void Index::set_nnz(std::uint64_t n) noexcept { _nnz = n; }
...
std::uint64_t Index::chrom_to_bin1_offset(std::string_view chrom_name) const {
  return at(chrom_name).front();
}

std::uint64_t Index::chrom_to_bin1_offset(std::uint32_t chrom_id) const {
...
void Index::finalize(std::uint64_t nnz) {
  ...
  std::for_each(_idx.rbegin(), _idx.rend(), [&](auto &it) {
    ...
    std::transform(offsets.rbegin(), offsets.rend(), offsets.rbegin(), [&fill_value](auto &offset) {
      if (offset == offset_not_set_value) {
        return fill_value;
      }
...
auto Index::init(const Reference &chroms, const BinTable &bins,
