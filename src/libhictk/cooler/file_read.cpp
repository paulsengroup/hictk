...
namespace {
template <typename T>
void read_or_throw(const hictk::cooler::RootGroup &root_grp, std::string_view key, T &buff) {
  try {
    buff = hictk::cooler::Attribute::read<T>(root_grp(), key);
...
template <typename T>
bool read_optional(const hictk::cooler::RootGroup &root_grp, std::string_view key, T &buff,
                   bool missing_ok) {
  // NOLINTBEGIN(bugprone-unchecked-optional-access)
  if (missing_ok && !hictk::cooler::Attribute::exists(root_grp(), key)) {
    return false;
  }

  return root_grp.try_read_attribute(key, buff);
}

template <typename T>
bool read_sum_optional(const hictk::cooler::RootGroup &root_grp, std::string_view key, T &buff,
                       bool missing_ok) {
  // NOLINTBEGIN(bugprone-unchecked-optional-access)
  if (missing_ok && !hictk::cooler::Attribute::exists(root_grp(), key)) {
    return false;
  ...
  return root_grp.try_read_sum_attribute(key, buff);
}
...
PixelSelector File::fetch(const balancing::Method &normalization_, bool load_index) const {
  return fetch(normalization_ptr(normalization_), load_index);
}
PixelSelector File::fetch(std::string_view range, const balancing::Method &normalization_,
                          QUERY_TYPE query_type) const {
...
PixelSelector File::fetch(std::string_view range1, std::string_view range2,
                          const balancing::Method &normalization_, QUERY_TYPE query_type) const {
...
bool File::purge_weights(std::string_view name) {
  if (_weights.empty()) {
    return false;
  }
  if (name.empty()) {
...
auto File::open_datasets(const RootGroup &root_grp, std::size_t cache_size_bytes, double w0)
    -> DatasetMap {
...
auto File::read_standard_attributes(const RootGroup &root_grp, bool initialize_missing)
  ...
  read_or_throw(root_grp, "format-version", attrs.format_version);
  read_or_throw(root_grp, "format", attrs.format);

  // Read mandatory attributes for Cooler v3
  auto missing_ok = attrs.format_version < 3;

  std::optional<std::string> bin_type{"fixed"};
  read_optional(root_grp, "bin-type", bin_type, missing_ok);
  attrs.bin_type = bin_type.value() == "fixed" ? BinTable::Type::fixed : BinTable::Type::variable;
  if (attrs.bin_type == BinTable::Type::fixed) {
    read_or_throw(root_grp, "bin-size", attrs.bin_size);
  }
  read_optional(root_grp, "storage-mode", attrs.storage_mode, missing_ok);
  ...
  missing_ok = true;
  read_optional(root_grp, "creation-date", attrs.creation_date, missing_ok);
  read_optional(root_grp, "format-url", attrs.format_url, missing_ok);
  read_optional(root_grp, "generated-by", attrs.generated_by, missing_ok);

  if (!read_optional(root_grp, "genome-assembly", attrs.assembly, missing_ok)) {
    read_optional(root_grp, "assembly", attrs.assembly, missing_ok);
  }

  read_optional(root_grp, "metadata", attrs.metadata, missing_ok);

  // Try to read other common attributes
  read_optional(root_grp, "nbins", attrs.nbins, missing_ok);
  read_optional(root_grp, "nchroms", attrs.nchroms, missing_ok);
  read_optional(root_grp, "nnz", attrs.nnz, missing_ok);

  read_sum_optional(root_grp, "sum", attrs.sum, missing_ok);
  read_sum_optional(root_grp, "cis", attrs.cis, missing_ok);
...
BinTable File::init_bin_table(const DatasetMap &dsets, BinTable::Type bin_type,
...
Index File::init_index(const Dataset &chrom_offset_dset, const Dataset &bin_offset_dset,
...
void File::read_index_chunk(std::initializer_list<Chromosome> chroms) const {
  read_index_chunk(chroms.begin(), chroms.end());
}

bool File::check_sentinel_attr() { return File::check_sentinel_attr(_root_group()); }

Bin File::get_last_bin_written() const {
