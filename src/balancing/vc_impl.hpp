...
inline VC<N>::VC(PixelIt first_pixel, PixelIt last_pixel, std::size_t num_rows,
  ...
  std::visit(
      [&](auto& sum) {
        std::for_each(first_pixel, last_pixel, [&](const Pixel<N>& p) {
          ...
          const auto bin1_id = p.coords.bin1.id() - bin_id_offset;
          const auto bin2_id = p.coords.bin2.id() - bin_id_offset;
  ...
  std::visit(
      [&](auto& sum) {
        using T = remove_cvref_t<decltype(sum)>;
        const auto& row_sum = std::get<std::vector<T>>(_rowsum);
        std::for_each(first_pixel, last_pixel, [&](const Pixel<N>& p) {
          if constexpr (std::is_floating_point_v<N>) {
            if (std::isnan(p.count)) {
              return;
          ...
          const auto rs1 = conditional_static_cast<double>(row_sum[bin1_id]);
          const auto rs2 = conditional_static_cast<double>(row_sum[bin2_id]);
          if (rs1 == 0 || rs2 == 0) {
            return;
          }
